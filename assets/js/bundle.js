(function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(require,module,exports){
  ...
  var app = {
    ...
    getListsFromAPI: async () => {
      try {
        let response = await fetch(utils.base_url+"/lists");
  ...
  const cardModule = {
    ...
    handleAddFormSubmit: async (event) => {
      ...
      try {
        let response = await fetch(utils.base_url+'/cards',{
          method: "POST",
          body: data
        });
        if (response.status != 200) {
    ...
    handleEditCardForm: async (event) => {
      ...
      //appeler l'API
      try {
        let response = await fetch(utils.base_url+'/cards/'+cardId,{
          method: "PATCH",
          body: data
        });
        if (response.status !== 200) {
  ...
  const listModule = {
    ...
    handleEditListForm: async (event) => {
      ...
      //appeler l'API
      try {
        let response = await fetch(utils.base_url + '/lists/' + listId, {
          method: "PATCH",
          body: data
        });
        if (response.status !== 200) {
    ...
    updateAllCards: (cards, listId) => {
      cards.forEach((card, position) => {
        ...
        fetch(utils.base_url + '/cards/' + cardId, {
          method: "PATCH",
          body: data
        });
      });
  ...
  const tagModule = {
    ...
    showAssociateModal: async (event) => {
      ...
      try {
        let response = await fetch(utils.base_url + '/tags');
        if (response.ok) {
    ...
    handleAssociateTag: async (event) => {
      ...
      try {
        ...
        let response = await fetch(utils.base_url + `/cards/${cardId}/tags`, {
          method: "POST",
          body: data
        });
        if (response.ok) {
    ...
    disassociateTag: async (event) => {
      ...
      const cardId = event.target.getAttribute('card-id');
      try {
        let response = await fetch(utils.base_url + `/cards/${cardId}/tags/${tagId}`, {
          method: "DELETE"
        });
        if (response.ok) {
    ...
    handleEditTag: async (event) => {
      ...
      try {
        let response = await fetch(utils.base_url + '/tags/' + tagId, {
          method: "PATCH",
          body: data
        });
        if (response.ok) {
  ...
  // un tout petit module, qui contient les propriétés et les fonctions dont tout le monde a besoin 
